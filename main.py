...
class MainWindow(QTabWidget):
    def __init__(self):
        ...
        self.model = self.model_load(weights="runs/best.pt",
    ...
    ***检测图片***
    ...
    def detect_img(self):
        ...
        source = self.img2predict  # 检测模式
        imgsz = [640, 640]  # 图片输入
        conf_thres = 0.7  # 置信度阈值
        iou_thres = 0.1  # NMS 降低可以降低重复
        max_det = 1000
        view_img = False
        save_crop = False
        nosave = False
        ...
        if source == "":
            ...
            source = str(source)
            device = select_device(self.device)# cuda device, i.e. 0 or 0,1,2,3 or cpu
            webcam = False
            stride, names, pt, jit, onnx = model.stride, model.names, model.pt, model.jit, model.onnx
